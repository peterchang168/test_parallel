...
class S3Handler(object):
    ...
    def cp_local_file_to_s3(self, bucket_name, src_path, dst_key , customer_sse_key=None , encrypt_algm='AES256', kwargs={}):
       	raise Exception('copy local file to s3 error') 
    ...
    def list_bucket_content(self, bucket_name, prefix = None, kwargs = {}):
        file_list = []
        opt_timestamp = int(time.time())
        for i in xrange(2):
            opt_timestamp -= 3600
            opt_time = new_domain_service_util.timestamp2str(opt_timestamp, '%Y%m%d%H')
            ptn_time = new_domain_service_util.get_format_time(opt_time, '%Y%m%d%H', new_domain_service_common.REDISDUMP_TIME_FORMAT)
            ptn_file = "dump.rdb.%s.gz" %ptn_time
            ptn_path = os.path.join("new_domain_snapshot", ptn_file)
            file_list.append(ptn_path)
        return file_list
